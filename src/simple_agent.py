...
class SimplePedestrian(Agent):
    ...
    def __init__(
        ...
        self.smoke_recovery_rate = (
            smoke_recovery_rate  # rate at which smoke exposure decreases per step
        )
    ...
    def resolve_overlaps(self, iterations=3):
        ...
        for _ in range(iterations):
            ...
            for other in self.model.space.get_neighbors(
                (self.x, self.y), R_query, include_center=False
            ):
            ...
            for wall in self.model.walls:
                gx, gy = self._project_to_line(
                    self.x, self.y, wall[0], wall[1], wall[2], wall[3]
                )
                dx = self.x - gx
                dy = self.y - gy
                dist = math.hypot(dx, dy)
    ...
    def _segments_intersect(self, p1, p2, q1, q2):
        (ax, ay), (bx, by) = p1, p2
        (cx, cy), (dx, dy) = q1, q2
        return self._ccw(ax, ay, cx, cy, dx, dy) != self._ccw(
            bx, by, cx, cy, dx, dy
        ) and self._ccw(ax, ay, bx, by, cx, cy) != self._ccw(ax, ay, bx, by, dx, dy)
    ...
    def update_smoke_exposure(self):
        ...
        if fire and fire.is_inside_smoke((self.x, self.y), self.r):
            ...
            if self.smoke_recovery_rate > 0.0:
                self.smoke_exposure = max(
                    0.0, self.smoke_exposure - self.smoke_recovery_rate * dt
                )
    ...
    def _is_passing_through_exit(self, buffer=0.3):
        ...
        for exit_x0, exit_y0, exit_x1, exit_y1 in self.model.exits:
            ...
            gx, gy = self._project_to_line(
                self.x, self.y, exit_x0, exit_y0, exit_x1, exit_y1
            )
